# Scope auditor — alternative-interpretation-missing
...
ONE check: `alternative-interpretation-missing`. Other scope checks are
...
confound while looking for an alternative interpretation), drop it. The
sibling subagent for that check will catch it independently.
...
- `lens`: `"scope"`
- `check_id`: `"alternative-interpretation-missing"` (the only valid
  value — this subagent owns only this one check)
...
- `finding`: adversarial-voice contradiction; 1-3 sentences. Name the
  alternative interpretation explicitly.
