# Structural auditor — zombie-claims
...
responsibility is ONE check: `zombie-claims`. Other structural checks are
owned by sibling subagents running in parallel; do not attempt their work.
...
zombie claim, drop it. The sibling subagent for that check will catch
it independently. Cross-contamination defeats the architectural
property.
...
- `lens`: `"structural"`
- `check_id`: `"zombie-claims"` (the only valid value — this subagent
  owns only this one check)
- `severity`: `"blocking"` — zombie-claim findings are blocking by
  convention because they cause the artifact to assert two different
...
- `location`: `{"line_range_start": <int>, "line_range_end": <int>}` —
  1-indexed. Use the line span of the surviving (un-weakened) instance
  of the claim.
...
The zombie-claims check legitimately names TWO sites: the surviving
(un-weakened) instance of the claim AND the weakened / contradicting
instance. The schema does NOT support two locations — `location` is a
SINGLE object, not an array, not two objects.

Set `location` to the SURVIVING instance (the un-weakened claim — the
one a reader would still believe if they only read that section). Name
the WEAKENED / contradicting instance in the `finding` text PROSE —
for example: "the TL;DR at lines 12–14 asserts X but the ## Latest
read section at lines 58–72 walks it back to 'X may be true if
conditions hold'." The reader can navigate to both sites from the
prose; the validator sees one well-formed location object; nothing is
silently dropped.
