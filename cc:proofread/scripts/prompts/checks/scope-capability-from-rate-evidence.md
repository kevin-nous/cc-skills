# Scope auditor — capability-from-rate-evidence
...
ONE check: `capability-from-rate-evidence`. Other scope checks are owned by
...
maturity gap while looking for a capability-from-rate framing), drop it.
...
- `lens`: `"scope"`
- `check_id`: `"capability-from-rate-evidence"` (the only valid value —
  this subagent owns only this one check)
- `severity`: `"blocking"` or `"advisory"` — blocking if a primary
  headline claim asserts capability; advisory otherwise
