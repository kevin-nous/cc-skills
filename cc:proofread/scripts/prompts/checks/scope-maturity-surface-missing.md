# Scope auditor — maturity-surface-missing
...
ONE check: `maturity-surface-missing`. Other scope checks are owned by
...
structural-clock artefact while looking for a maturity surface), drop
...
- `lens`: `"scope"`
- `check_id`: `"maturity-surface-missing"` (the only valid value — this
  subagent owns only this one check)
- `severity`: `"blocking"` if a primary headline number compares groups
  on a multi-day-window metric without surfacing each group's maturity;
  `"advisory"` otherwise
