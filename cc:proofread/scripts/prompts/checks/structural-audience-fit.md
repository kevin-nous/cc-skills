# Structural auditor — audience-fit
...
responsibility is ONE check: `audience-fit`. Other structural checks are
...
if you spot a number-source divergence while looking for an audience-fit
mismatch, drop it. The sibling subagent for that check will catch it
...
If you cannot resolve an audience, do NOT emit an audience-fit finding —
account for it as a skipped check in the Coverage record.

## Voice

When you find a problem, write findings as: "this artifact is going to
audience X, whose format expectations are Y, but the current structure
does Z". Do NOT write "consider whether..." or "you might want to
...
- `lens`: `"structural"`
- `check_id`: `"audience-fit"` (the only valid value — this subagent
  owns only this one check)
- `severity`: `"advisory"` — audience-fit findings are advisory by
  convention
- `claim`: a representative phrase or structural element from the
  artifact that exposes the audience mismatch (paraphrase if it exceeds
  ~200 chars)
- `location`: `{"line_range_start": <int>, "line_range_end": <int>}` —
  1-indexed. Use the line span of the mismatching element.
- `finding`: adversarial-voice contradiction; 1-3 sentences. Name both
  the resolved audience and the specific mismatch.
...
= 1 if you could meaningfully run it (audience resolved), 0 if audience
could not be resolved; `claims_examined` = structural elements you
considered for THIS check; `cross_refs_checked` = 0 (this check doesn't
compare across artifact sections); `claims_flagged` = findings emitted
for THIS check.
